import "../../styles/components/match/ClinicalMatchCard.css";
import React, { useState } from "react";
import { useMatch } from "../../context/MatchContext";
import { CheckIcon, MatchIcon } from "../../components/ui/Icons";

export default function ClinicalMatchCard({
  role = "recipient",
  onNavigateTab,
}: {
  role?: string;
  ...
  const [locationInput, setLocationInput] = useState(
    "Philippine General Hospital (PGH) - Organ Transplant Center",
  );
  ...
  const partnerName =
    role === "donor"
      ? `${match.recipient.first_name} ${match.recipient.last_name}`
  ...
  const isProposedBySelf =
    hasProposal && match.proposedSchedule?.proposedBy === selfRole;
  const isProposedByOther =
    hasProposal && match.proposedSchedule?.proposedBy !== selfRole;
  ...
  const handleConfirm = () => {
    if (match.proposedSchedule) {
      setScheduledDate(
        match.proposedSchedule!.date || dateInput,
        match.proposedSchedule!.time || timeInput,
        match.proposedSchedule!.location || "",
      );
    }
  };
  ...
  if (match.status === "rejected") {
    return (
      <div className="card anim-in clinical-match-rejection-card">
        <h3 className="clinical-match-rejection-title">Demo match declined</h3>
        <p className="clinical-match-rejection-copy">
          No new match search has started. Choose Start over to review another
          sample match.
        </p>
    ...
    <div className="card anim-in match-card clinical-match-match-card-shell">
      ...
      <div className="match-header clinical-match-match-header">
        ...
        <div className="clinical-match-match-eyebrow">
          <span
            className={`clinical-match-status-badge badge ${match.status === "pending_hospital_approval" ? "badge-warning" : "badge-success"}`}
          >
            {match.status === "pending_hospital_approval" ? (
              "Hospital demo review pending"
            ) : (
              <>
                <span
                  className="clinical-match-status-check"
                  aria-hidden="true"
                >
                  <CheckIcon size={12} />
                </span>{" "}
          ...
          <span className="match-header-score clinical-match-score-wrap">
            <span className="clinical-match-score-value">
        ...
        <h2 className="clinical-match-title">Your {match.organ} match.</h2>

        {/* Hospital attribution - quiet, with hairline rule */}
        <p className="clinical-match-hospital-attribution">
          <span>at</span>
          <strong className="clinical-match-score-label">
            {match.hospital.name}
          </strong>
      ...
      <div className="clinical-match-match-details">
        <div className="clinical-match-detail-organ">
          <div className="clinical-match-detail-label">
            What you're donating / receiving
          </div>
          <div className="clinical-match-detail-value">{match.organ}</div>
          <div className="clinical-match-detail-note">
            Compatibility estimate / not clinical clearance
          </div>
        </div>
        <div className="clinical-match-detail-partner">
          <div className="clinical-match-partner-label">
            {partnerRole}'s name
          </div>
          ...
          <div className="clinical-match-partner-note">
            {isSigned
              ? "You can both see each other now"
              : "Their name shows up after you sign"}
          </div>
        </div>
        <div className="clinical-match-detail-priority">
          <div className="clinical-match-priority-label">Priority</div>
          <div className="clinical-match-priority-value">
            {match.urgencyLevel ? match.urgencyLevel : "Urgent"}
          </div>
      ...
      {match.status === "pending_hospital_approval" && (
        <div
          className="clinical-match-review-pending"
          role="status"
          aria-live="polite"
        >
          <div>
            ...
            <div className="clinical-match-review-copy">
              A hospital administrator is reviewing this demo match. This screen
              does not provide clinical clearance or guarantee a text message.
            </div>
          </div>
        </div>
      )}
      ...
      {[
        ...
        <div className="clinical-match-appointment-section">
          <div className="clinical-match-appointment-header">
            <span
              className="clinical-match-appointment-icon"
              aria-hidden="true"
            >
              <MatchIcon size={22} />
            </span>
            <div className="clinical-match-appointment-copy">
              <div className="clinical-match-appointment-title">
                Appointment is set
              </div>
              <div className="clinical-match-appointment-status">
                <span
                  className="clinical-match-appointment-check"
                  aria-hidden="true"
                >
                  <CheckIcon size={12} />
                </span>{" "}
                Schedule saved in this demo
              </div>
            </div>
            {isSigned ? (
          ...
          <div className="clinical-match-appointment-grid">
            ...
            <div className="clinical-match-next-card">
              <div>
                <div className="clinical-match-next-label">Next step</div>
              ...
              <div>
                {onNavigateTab && (
                  <button
                    ...
                    onClick={() =>
                      onNavigateTab(isSigned ? "chat" : "agreement")
                    }
                  >
      ...
      {![
        ...
        <div className="clinical-match-schedule-section">
          <div className="clinical-match-schedule-heading">
            <span className="clinical-match-schedule-icon" aria-hidden="true">
              <MatchIcon size={22} />
            </span>
            <div className="clinical-match-schedule-copy">
              ...
              <p className="clinical-match-schedule-description">
                The hospital review demo approved this match. Suggest a date
                below; a doctor must make any clinical decision.
              </p>
            </div>
          </div>
          ...
          {isProposedByOther && !showCounterForm ? (
            <div className="clinical-match-schedule-heading">
              ...
              <div className="clinical-match-schedule-copy">
                <div>
                  ...
                  <div className="clinical-match-schedule-description">
                    {match.proposedSchedule!.date}
                  </div>
                </div>
                <div>
                  <div className="clinical-match-proposal-card">Time</div>
                  <div className="clinical-match-proposal-heading">
                    {match.proposedSchedule!.time}
                  </div>
                </div>
                <div>
                  <div className="clinical-match-proposal-grid">Where</div>
                  <div className="clinical-match-proposal-label-date">
                    {match.proposedSchedule!.location}
                  </div>
                </div>
              </div>
              ...
              >
                <span
                  className="clinical-match-proposal-label-time"
                  aria-hidden="true"
                >
                  <CheckIcon size={14} />
                </span>{" "}
              ...
              <button
                className="clinical-match-proposal-time"
                type="button"
            ...
            <div className="clinical-match-proposal-label-location">
              ...
              <div className="clinical-match-proposal-confirm">
                <div>
                  <div className="clinical-match-proposal-action-icon">
                    Date
                  </div>
                  <div className="clinical-match-proposal-change">
                    {match.proposedSchedule!.date}
                  </div>
                </div>
                <div>
                  <div className="clinical-match-waiting-card">Time</div>
                  <div className="clinical-match-waiting-heading">
                    {match.proposedSchedule!.time}
                  </div>
                </div>
                <div>
                  <div className="clinical-match-waiting-grid">Where</div>
                  <div className="clinical-match-waiting-label-date">
                    {match.proposedSchedule!.location}
                  </div>
                </div>
              </div>
              <p className="clinical-match-waiting-date">
                A simulated notification was queued in this demo. Check back
                here for their response.
              </p>
              <button
                className="clinical-match-waiting-label-time"
                type="button"
                onClick={() => setShowCounterForm(true)}
              >
                Change the date
            ...
            <form
              className="clinical-match-waiting-time"
            ...
            >
              ...
              <div className="clinical-match-waiting-location">
                <div>
                  <label
                    className="clinical-match-waiting-copy"
                    htmlFor="clinical-date"
                  >
                    Date
                  </label>
                  <input
                ...
                <div>
                  <label
                    className="clinical-match-waiting-change"
                    htmlFor="clinical-time"
                  >
                    Time
                  </label>
                  <select
                    id="clinical-time"
                    className="input clinical-match-schedule-time-input"
                    value={timeInput}
                ...
                <div className="clinical-match-score-value">
                  <label
                    className="clinical-match-schedule-form"
                    htmlFor="clinical-location"
                  >
                    Where
                  </label>
                  <input
              ...
              <button
                type="submit"
                className="btn btn-primary btn-lg btn-full clinical-match-schedule-form-title"
              >
                <span
                  className="clinical-match-schedule-fields"
                  aria-hidden="true"
                >
                  <CheckIcon size={14} />
                </span>{" "}
              ...
              {showCounterForm && (
                <button
                  className="clinical-match-schedule-label-date"
                  type="button"
