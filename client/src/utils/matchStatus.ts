...
export type MatchStatus = keyof typeof STATUS_LABELS | string;
export function formatStatus(s: MatchStatus | null | undefined) {
  if (!s) return "—";
  return (STATUS_LABELS as Record<string, string>)[s] || s.replace(/_/g, " ");
}

export function statusTone(s: MatchStatus) {
  return (STATUS_TONES as Record<string, string>)[s] || "muted";
}
...
export function statusPillClass(s: MatchStatus) {
  const tone = (PILL_BY_STATUS as Record<string, string>)[s] || "info";
...
export function lifecycleStepIndex(s: MatchStatus) {
  switch (s) {
