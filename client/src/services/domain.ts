...
export const STATIC_MATCHES: MatchItem[] = [
  {
    id: "match-pgh-002",
    donor: "Rosa Magtanggol",
...
export const filterMatches = (allMatches: MatchItem[]) => ({
  ...
  pendingCases: allMatches.filter(
    (m: MatchItem) => m.status === "pending_hospital_approval",
  ),
  activeCases: allMatches.filter(
    (m: MatchItem) =>
      m.status !== "pending_hospital_approval" && m.status !== "rejected",
  ),
  rejectedCases: allMatches.filter((m: MatchItem) => m.status === "rejected"),
});
...
export const generateScheduleSlots = (
  ...
  for (let i = 0; i < slotCount; i++) {
    ...
    const hospitals =
      type === "organ"
        ? [
            "PGH - Organ Transplant Unit",
            "St. Luke's BGC - Transplant Center",
            "NKTI - Kidney Transplant",
            "Heart Center - Cardiothoracic",
          ]
        : [
            "Philippine Red Cross - Blood Center",
            "PGH - Blood Bank",
            "St. Luke's - Apheresis Unit",
            "RITM - Blood Services",
          ];

    const hospital =
      hospitals[Math.floor(Math.random() * hospitals.length)] ?? hospitals[0];

    slots.push({
      id: `slot-${type}-${i}-${Date.now()}`,
      start: slotDate.toISOString(),
      end: new Date(
        slotDate.getTime() + (type === "organ" ? 3 : 1.5) * 60 * 60 * 1000,
      ).toISOString(),
      type:
        type === "organ"
          ? "Tri-party Surgical Consultation"
          : "Blood Donation & Compatibility Verification",
      location: hospital,
...
export const generateDonorSlots = (
  type: string,
  startDate: Date,
  endDate: Date,
): ScheduleSlot[] => {
...
export const validateRecipientParams = (params: {
  ...
  if (!params.blood_type_needed)
    newErrors.blood_type_needed = "Blood type is required";
  if (params.request_type === "organ" && !params.organ_needed)
    newErrors.organ_needed = "Organ type is required";
  return newErrors;
};
