...
export type MatchRecord = {
  ...
  hospital: { name: string; [key: string]: unknown };
  [key: string]: unknown;
};
...
export function getInitialMatch(
...
export function parseStorageEventValue(newValue: string | null): MatchRecord {
...
export function getInitialStaticMatches<T>(
  fallback: T[],
  storage: Storage | null = typeof window !== "undefined"
    ? window.localStorage
    : null,
...
export function saveStaticMatchesToStorage<T>(
  matches: T[] | null,
  storage: Storage | null = typeof window !== "undefined"
    ? window.localStorage
    : null,
...
export function clearStaticMatchesFromStorage(
  storage = typeof window !== "undefined" ? window.localStorage : null,
  dispatchEvent = true,
) {
  if (!storage) return;
...
export function parseStaticMatchesStorageEvent<T>(
...
export function calculateUpdatedMatchFromProfile(
  match: MatchRecord,
  role: "donor" | "recipient",
  profileFields: ProfileFields,
): { success: boolean; error?: string; updatedMatch?: MatchRecord } {
  if (!match || !role || !profileFields) {
    return {
      success: false,
      error: "Invalid parameters provided for match update.",
    };
  }
  const updatedMatch = JSON.parse(JSON.stringify(match)) as MatchRecord;
  ...
  if (role === "donor") {
    ...
    if (organs !== undefined) {
      ...
      const hasMatching =
        ...
        organs.some((o) => o.toLowerCase() === recipientOrgan.toLowerCase());
      if (!hasMatching) {
        return {
          success: false,
          error:
            "Validation Error: Organ pledged does not match recipient anatomical need.",
        };
      }
    }
    ...
    if (
      avail === false &&
      !["rejected", "ready_for_transplant"].includes(updatedMatch.status)
    ) {
      return {
        success: false,
        error:
    ...
    if (organs !== undefined) {
      ...
      const matched = organs.find(
        (o) =>
          o.toLowerCase() ===
          (updatedMatch.recipient?.organ_needed || "").toLowerCase(),
      );
      if (matched) {
