...
function Toast({ toast, onRemove }) {
  ...
  const isEMessage = options.variant === 'emessage';
  ...
  const icons = {
    ...
    emessage: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="5" y="2" width="14" height="20" rx="2" ry="2"/><line x1="12" y1="18" x2="12.01" y2="18"/></svg>,
  };
  ...
  const classNames = {
    ...
    info: 'toast-info',
    emessage: 'toast-emessage',
  };

  const renderType = isEMessage ? 'emessage' : type;

  return (
    <div
      className={`toast ${classNames[renderType]} ${exiting ? 'toast-exit' : ''}`}
      style={{ animation: exiting ? 'toastOut 0.2s ease forwards' : 'toastIn 0.3s var(--ease-up)' }}
      role="alert"
      aria-live="assertive"
    >
      <div className="toast-icon">{icons[renderType]}</div>
