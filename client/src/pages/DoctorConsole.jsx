...
const CASES = [
  { id:'case-001', donor:'Juan Dela Cruz',  recipient:'Ana Reyes',     type:'blood', match:'O- â†’ A+',  urgency:'urgent',   score:102 },
  { id:'case-002', donor:'Rosa Magtanggol', recipient:'Carlos Santos', type:'organ', match:'AB- â†’ AB-', urgency:'critical', score:98  },
  { id:'case-003', donor:'Pedro Reyes',     recipient:'Luz Garcia',    type:'blood', match:'A+ â†’ A+',  urgency:'moderate', score:90  },
];
...
export default function DoctorConsole() {
  ...
  const TABS = [
    ...
    { id:'consent',  label:'Blockchain Consent',  icon:<ChainIcon />  },
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--background)' }}>
      ...
      {/* â”€â”€ Hero â”€â”€ (navy card style from the site for stats row) */}
      <section className="hero">
        ...
        <div className="container" style={{ position:'relative' }}>
          ...
          {/* Navy credential card â€” mirrors the API key card on platforms.e.gov.ph */}
          <div className="card card-navy anim-up-d3" style={{ display:'inline-flex', alignItems:'center', gap:18, padding:'20px 28px', marginTop:28, maxWidth:440 }}>
            <div style={{ width:44,height:44,borderRadius:12,background:'rgba(255,255,255,0.15)',display:'flex',alignItems:'center',justifyContent:'center' }}>
              <ShieldCheckIcon />
            </div>
            <div>
              <div style={{ fontSize:11,fontWeight:700,letterSpacing:'0.07em',textTransform:'uppercase',color:'rgba(255,255,255,0.65)',marginBottom:4 }}>DICT eGov Authorization</div>
              <div style={{ fontFamily:'var(--font-mono)',fontSize:16,fontWeight:600,letterSpacing:'-0.02em' }}>egov_ehealth_â€¢â€¢â€¢â€¢7c2a</div>
            </div>
            <div style={{ marginLeft:'auto',display:'flex',alignItems:'center',gap:6,background:'rgba(255,255,255,0.1)',padding:'4px 12px',borderRadius:'var(--r-full)',fontSize:11,fontWeight:600 }}>
        ...
        <div className="container">
                  ...
                  {pending.map(c => (
                    <div key={c.id} className="card anim-up" style={{ display:'flex', alignItems:'center', gap:18, flexWrap:'wrap', padding:'18px 22px' }}>
                      <div style={{ flex:1, minWidth:200 }}>
                        ...
                        <div style={{ fontWeight:700,fontSize:15 }}>{c.donor} <span style={{color:'var(--foreground-muted)',fontWeight:400}}>â†’</span> {c.recipient}</div>
                        <div style={{ fontSize:12,color:'var(--foreground-muted)',marginTop:3 }}>Match: {c.match} Â· Case {c.id}</div>
                      </div>
                ...
                <>
                  ...
                  {resolved.map(c => (
                    <div key={c.id} className="card anim-up" style={{ display:'flex', alignItems:'center', gap:18, padding:'14px 22px', border:'1px solid rgba(5,150,105,0.25)', background:'rgba(5,150,105,0.02)', opacity:0.9 }}>
                      <span className="badge badge-verified">âœ“ Approved</span>
                      <div style={{ flex:1, fontWeight:600, fontSize:14 }}>{c.donor} â†’ {c.recipient}</div>
            ...
            <div style={{ maxWidth:700, margin:'0 auto', display:'flex', flexDirection:'column', gap:16 }}>
                ...
                <div style={{ display:'flex',alignItems:'center',gap:14,marginBottom:16 }}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:12,color:'var(--foreground-muted)',marginTop:2}}>
                      {selCase ? `Case: ${selCase.donor} â†’ ${selCase.recipient}` : 'Select a case from the Cases tab to auto-fill'}
                    </div>
                  </div>
                </div>
              ...
              {loading && (
                <div className="empty-state">
                  ...
                  <p>AI is computing optimal slotsâ€¦</p>
                </div>
              )}
          ...
          {tab === 'consent' && (
            <div style={{ maxWidth:540, margin:'0 auto', display:'flex', flexDirection:'column', gap:14 }}>
              {selCase && (
                <div className="card card-xs" style={{ background:'var(--primary-10)', border:'1px solid rgba(0,56,168,0.15)', fontSize:14 }}>
                  <span style={{fontWeight:700}}>Case: </span>{selCase.donor} â†’ {selCase.recipient} Â· {selCase.match}
                </div>
              )}
              <BlockchainBadge matchId={selCase?.id||'demo-match-001'} donorId="donor-001" recipientId="recipient-001" />
            </div>
          )}
...
function ShieldCheckIcon() { return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg>; }
