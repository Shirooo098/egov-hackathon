...
import type { MatchRecord } from "../context/matchHelpers";

type DonorPledge = { bloodType?: string; isBlood?: boolean; organs?: string[] };
type DonorDashboardProps = { onboardingPledge?: DonorPledge };
...
export default function DonorDashboard({
  ...
  const { match, hospitalApproved, consentSigned, saveIntake, platform } =
    useMatch();
  ...
  const donorEpisodeId = platform.cases.find(
    (item) => item.role === "donor",
  )?.episodeId;
  const intake = donorEpisodeId ? platform.intakes[donorEpisodeId] : undefined;
  useEffect(() => {
    if (!intake) return;
    if (typeof intake.declaredBloodGroup === "string")
      setBloodType(intake.declaredBloodGroup);
    if (typeof intake.bloodDonor === "boolean") setIsBlood(intake.bloodDonor);
    if (Array.isArray(intake.pledgedOrgans))
      setOrgans(
        intake.pledgedOrgans.filter((v): v is string => typeof v === "string"),
      );
    if (typeof intake.availability === "string")
      setAvail(intake.availability === "available");
  }, [intake]);

  const handleAvailChange = (
    valOrFn: boolean | ((value: boolean) => boolean),
  ) => {
  ...
  const saveProfile = async () => {
    try {
      await saveIntake("donor", {
        declaredBloodGroup: bloodType,
        pledgedOrgans: organs,
        bloodDonor: isBlood,
        availability: avail ? "available" : "unavailable",
        state: "active",
      });
      ...
      warning(
        ...
        { title: "Profile Sync Warning", duration: 5000 },
      );
    }
  };
