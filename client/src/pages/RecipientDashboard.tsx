...
type RecipientDashboardProps = { onboardingHealth?: RecipientHealth };
...
export default function RecipientDashboard({
  ...
  const [requestType, setRequestType] = useState(
    onboardingHealth?.request_type || "organ",
  );
  const [bloodTypeNeeded, setBloodTypeNeeded] = useState(
    () =>
      match.recipient?.blood_type_needed ||
      onboardingHealth?.blood_type_needed ||
      "B+",
  );
  const [organNeeded, setOrganNeeded] = useState(
    () =>
      match.recipient?.organ_needed ||
      onboardingHealth?.organ_needed ||
      "Kidney",
  );
  const [urgencyLevel, setUrgencyLevel] = useState(
    () => match.recipient?.urgency || match.urgencyLevel || "urgent",
  );
  const { success, warning } = useToast();
  const recipientEpisodeId = platform.cases.find(
    (item) => item.role === "recipient",
  )?.episodeId;
  const intake = recipientEpisodeId
    ? platform.intakes[recipientEpisodeId]
    : undefined;
  useEffect(() => {
    if (!intake) return;
    if (typeof intake.requestType === "string")
      setRequestType(intake.requestType);
    if (typeof intake.declaredBloodGroup === "string")
      setBloodTypeNeeded(intake.declaredBloodGroup);
    setOrganNeeded(
      typeof intake.requestedOrgan === "string" ? intake.requestedOrgan : "",
    );
    if (typeof intake.urgency === "string")
      setUrgencyLevel(
        intake.urgency === "routine" ? "moderate" : intake.urgency,
      );
  }, [intake]);
  ...
  const saveProfile = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await saveIntake("recipient", {
        requestType,
        declaredBloodGroup: bloodTypeNeeded,
        requestedOrgan:
          requestType === "organ" ? organNeeded.toLowerCase() : null,
        urgency: urgencyLevel === "moderate" ? "routine" : urgencyLevel,
        state: "active",
      });
      success("Recipient medical preferences updated in this demo.", {
  ...
  const isScheduleUnlocked =
    isApproved || match.status !== "pending_hospital_approval";
  ...
  const TABS = [
    ...
    {
      id: "schedule",
      label: isScheduleUnlocked ? "Schedule" : "Schedule",
      shortLabel: "Schedule",
      icon: <CalIcon />,
      locked: !isScheduleUnlocked,
    },
    {
      id: "agreement",
      label: isAgreementUnlocked ? "Agreement" : "Agreement",
    ...
    {
      id: "chat",
      label: isChatUnlocked ? "Chat" : "Chat",
      shortLabel: "Chat",
      icon: <ChatIcon />,
      ...
      <section className="hero care-journey-hero">
        <div className="container">
          ...
          <h1 className="care-journey-title anim-up-d1">
            Recipient Care Journey
          </h1>
          <ul
            className="care-journey-rail anim-up-d3"
            aria-label="Recipient current care facts"
          >
            <li role="status">
            ...
            <li>
              ...
              <strong>{bloodTypeNeeded}</strong>
            </li>
            <li>
              <span>Organ need</span>
              <strong>{organNeeded}</strong>
            </li>
            <li>
              <span>Urgency</span>
              <strong className="care-journey-urgency">{urgencyLevel}</strong>
            </li>
          </ul>
        </div>
      </section>
      ...
      <div className="dashboard-network-band">
        <div className="marquee-outer">
          <div className="marquee-track dashboard-marquee-track">
            {[
              "DOH NATIONAL TRANSPLANT PROGRAM · DEMO",
              "PHILIPPINE GENERAL HOSPITAL (PGH) · DEMO",
              "Simulated trust registry",
              "Sample identity credential",
            ...
            <div className="dashboard-narrow-680">
              <div className="card anim-up recipient-profile-card">
                <div className="recipient-profile-header">
                  <div className="recipient-profile-avatar">C</div>
                  <div>
                ...
                <form onSubmit={saveProfile} className="recipient-profile-form">
                  <div className="recipient-profile-grid">
                    <div>
                      <label
                        htmlFor="recipient-dashboard-request-type"
                        className="recipient-field-label"
                      >
                        Medical Need Type
                      </label>
                      <select
                        id="recipient-dashboard-request-type"
                        className="input recipient-field-control"
                        value={requestType}
                        onChange={(e) => setRequestType(e.target.value)}
                      >
                        <option value="organ">
                          Anatomical Organ Transplantation
                        </option>
                        <option value="blood">
                    ...
                    <div>
                      <label
                        htmlFor="recipient-dashboard-blood-type"
                        className="recipient-field-label"
                      >
                        Required Blood Group
                      </label>
                      <select
                        id="recipient-dashboard-blood-type"
                        className="input recipient-field-control"
                  ...
                  {requestType === "organ" && (
                    <div>
                      <label
                        htmlFor="recipient-dashboard-organ"
                        className="recipient-field-label"
                      >
                        Target Anatomical Organ
                      </label>
                      <select
                        id="recipient-dashboard-organ"
                        className="input recipient-field-control"
                        value={organNeeded}
                  ...
                  <div>
                    <label
                      htmlFor="recipient-dashboard-urgency"
                      className="recipient-field-label"
                    >
                      Clinical Urgency &amp; Triage Level
                    </label>
                    <select
                      id="recipient-dashboard-urgency"
                      className="input recipient-field-control"
                      value={urgencyLevel}
                      onChange={(e) => setUrgencyLevel(e.target.value)}
                    >
                      <option value="moderate">
                        Moderate Priority - Outpatient Coordination
                      </option>
                      <option value="urgent">
                        Urgent Priority - Active Hospital Roster
                      </option>
                      <option value="critical">
                        Critical Priority - Immediate Surgical ICU Waitlist
                      </option>
                    </select>
                  </div>

                  <div className="recipient-notice">
                    ℹ️ Changes are saved as declared coordination preferences.
                    They do not imply compatibility, eligibility, or clinical
                    clearance.
                  </div>

                  <button
                    type="submit"
                    className="btn btn-primary btn-lg btn-full recipient-submit"
                  >
          ...
          {tab === "schedule" && (
            <div className="dashboard-narrow-780">
              {isScheduleUnlocked ? (
                <>
                  <div
                    className="card dashboard-section-gap"
                    aria-live="polite"
                  >
                    <h2>Hospital scheduling</h2>
                    <p>
                      Scheduling is available after mutual pair acceptance and
                      requires an assigned hospital reviewer proposal. No
                      citizen booking controls are available here.
                    </p>
                  </div>
                </>
              ) : (
