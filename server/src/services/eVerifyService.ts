...
type TokenCache = { token: string | null; expiresAt: number | null };
let tokenCache: TokenCache = { token: null, expiresAt: null };

/**
 * Simulate network delay for demo realism (800ms - 1500ms)
 */
function simulateDelay(minMs = 800, maxMs = 1500): Promise<void> {
 ...
 */
function generatePhilSysProfile(inputData: VerifyInput) {
...
async function getAccessToken() {
  ...
  try {
    ...
    console.warn('[eVerify] Network error, falling back to demo mode:', err instanceof Error ? err.message : String(err));
    tokenCache = {
      token: 'everify_access_' + createHash('sha256').update(Date.now().toString()).digest('hex').substring(0, 32),
 ...
 * POST /api/query
 */
async function verifyIdentity({ first_name, last_name, birth_date, middle_name, suffix, face_liveness_session_id }: VerifyInput) {
  // Demo mode path
  if (DEMO_MODE) {
 ...
 */
async function decodeQR(qr_value: string): Promise<Record<string, unknown>> {
  // Demo mode path
  if (DEMO_MODE) {
...
export { getAccessToken, verifyIdentity, decodeQR, DEMO_MODE };
