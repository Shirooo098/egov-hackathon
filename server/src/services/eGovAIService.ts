...
async function getAIToken() {
  ...
  try {
    ...
    console.warn('[eGovAI] Network error, falling back to demo mode:', err instanceof Error ? err.message : String(err));
    aiTokenCache = {
      token: 'egovai_demo_' + createHash('sha256').update(Date.now().toString()).digest('hex').substring(0, 32),
      expiresAt: now + 28800 * 1000,
      creditsRemaining: 999
 ...
 */
async function askLawsAndRegulations(prompt: string, category = 'PH') {
  // Demo mode path
  if (DEMO_MODE) {
 ...
 */
function generateLawsResponse(prompt: string, _category: string) {
 ...
 */
async function generateScheduleSlots({ hospitalAvailability, doctorAvailability, donorAvailability, recipientAvailability, urgencyLevel = 'moderate' }: ScheduleInput): Promise<ScheduleResult> {
  // Demo mode path
  if (DEMO_MODE) {
...
Return ONLY a valid JSON array of exactly 3 slot objects.`;
  ...
  try {
    const slots = JSON.parse(data.data ?? '[]') as Array<Record<string, unknown>>;
 ...
 */
function generateDemoSlots({ hospitalAvailability, doctorAvailability, donorAvailability, recipientAvailability, urgencyLevel = 'moderate' }: ScheduleInput): Array<Record<string, unknown>> {
  const now = new Date();
