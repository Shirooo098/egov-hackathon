...
type JsonRpcMethod = 'eth_blockNumber' | 'eth_chainId' | 'eth_gasPrice' | 'eth_getTransactionReceipt';
 ...
 */
function computeConsentHash(consentData: ConsentData): string {
 ...
 */
async function rpc(method: JsonRpcMethod, params: unknown[] = []): Promise<unknown> {
  if (DEMO_MODE) {
  ...
  if (data.error) throw new Error(`Besu RPC error: ${data.error.message || 'Unknown error'}`);
  return data.result;
}
 ...
 */
async function anchorConsent(consentData: ConsentData) {
...
async function getChainInfo() {
  ...
  try {
    ...
    return {
      ...
      blockNumber: parseInt(String(blockNumber), 16),
      gasPrice: parseInt(String(gasPrice), 16),
      rpcUrl: BESU_RPC,
      explorerUrl: 'https://hackathon-explorer.e.gov.ph'
    };
  } catch (err) {
