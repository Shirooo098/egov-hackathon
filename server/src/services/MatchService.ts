...
export type MatchDonor = { blood_type: string; donor_profile?: { organ_pledges?: string[]; availability_status?: string } | null; [key: string]: unknown };
export type RecipientRequest = { request_type: 'blood' | 'organ'; blood_type_needed: string; organ_needed?: string | null; urgency_level?: Urgency | string };
export type MatchResult = { donor: MatchDonor; rawScore: number; compatibilityScore: number; compatible: boolean };

// Urgency multiplier for scoring
const URGENCY_WEIGHT = { critical: 1.5, urgent: 1.2, moderate: 1.0 } as const;
 ...
 * @returns {boolean}
 */
function isBloodCompatible(donorType: string, recipientType: string) {
 ...
 */
function bloodMatchScore(donorType: string, recipientType: string) {
 ...
 */
function organMatchScore(donor: MatchDonor, recipientRequest: RecipientRequest) {
 ...
 */
function findMatches(recipientRequest: RecipientRequest, donors: MatchDonor[]): MatchResult[] {
  const urgencyMultiplier = URGENCY_WEIGHT[recipientRequest.urgency_level as Urgency] || 1.0;

  const results = donors
    .filter((d) => d.donor_profile?.availability_status === 'available')
    .map((donor): MatchResult => {
      let score = 0;
 ...
 */
function getCompatibleDonorTypes(recipientType: string): readonly string[] {
  return BLOOD_COMPATIBILITY[recipientType as BloodType] || [];
}
