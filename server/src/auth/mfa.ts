...
function keyFromEnvironment(env: NodeJS.ProcessEnv = process.env): Buffer {
  ...
  if (key.length !== 32)
    throw new Error("STAFF_MFA_ENCRYPTION_KEY must encode exactly 32 bytes");
  return key;
}
  ...
  for (let delta = -window; delta <= window; delta++) {
    ...
    if (
      ...
      crypto.timingSafeEqual(
        Buffer.from(totpAt(secret, counter)),
...
function generateTotp(secret: string, nowMs = Date.now()): string {
  return totpAt(secret, Math.floor(nowMs / 30_000));
}
function createEnrollmentSecret(random = crypto.randomBytes(20)): string {
  return base32Encode(random);
}
function createRecoveryCodes(
  count = 10,
  random: () => Buffer = () => crypto.randomBytes(18),
): string[] {
  return Array.from({ length: count }, () => random().toString("base64url"));
}
