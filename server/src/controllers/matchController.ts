...
async function getMatches(req: Request, res: Response, next: NextFunction) {
  try {
    const { request_type, blood_type_needed, organ_needed, urgency_level } =
      req.query;
    if (!request_type || !blood_type_needed) {
      return res
        ...
        .json({
          success: false,
          message: "request_type and blood_type_needed are required",
        });
    }
    const recipientRequest = {
      request_type: String(request_type) as "blood" | "organ",
      blood_type_needed: String(blood_type_needed),
      organ_needed: typeof organ_needed === "string" ? organ_needed : undefined,
      urgency_level:
        typeof urgency_level === "string" ? urgency_level : "moderate",
    };
...
async function getCompatibility(
  ...
  try {
    ...
    res.json({
      success: true,
      data: { blood_type, compatible_donor_types: compatible },
    });
  } catch (err) {
    next(err);
  }
