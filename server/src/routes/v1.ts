...
import verifyRouter from './verify.js'; import { createSessionRouter } from './session.js'; import staffRouter from './staff.js';
import rebaselineRouter from './rebaseline.js'; import platformRouter, { createCitizenPlatformRouter } from './platform.js'; import matchRouter from './match.js';
import scheduleRouter from './schedule.js'; import blockchainRouter from './blockchain.js'; import egovaiRouter from './egovai.js';
import egovRouter from './egov.js'; import emessageRouter from './emessage.js';
...
export function createV1Router(config: RuntimeConfig) {
  ...
  const unavailable = (_req: express.Request, res: express.Response) => res.status(404).json({ success: false, error: 'not_found', message: 'Route not found' });
  router.get(['/csrf', '/csrf-token'], csrfToken); router.get('/auth/csrf', csrfToken);
  router.get('/health', (_req, res) => res.json({ success: true, status: 'alive', mode: config.mode }));
  if (config.mode !== 'synthetic') router.use('/egov', unavailable);
  router.use('/auth', verifyRouter); router.use('/auth', createSessionRouter(config)); router.use('/auth/egov', createEgovAuthRouter(config)); router.use('/auth', staffRouter);
